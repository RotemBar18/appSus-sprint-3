...
function addBook(book) {
    let { title, subtitle, authors, publishedDate, description, pageCount, imageLinks } = book.volumeInfo
    let { thumbnail } = imageLinks
    book.isOnSale = (book.saleInfo.saleability === 'NOT_FOR_SALE') ? false : true
    var amount = Math.floor(Math.random() * ((200 - 10) + 1) + 10);
    description = (description)?  description:'This book does not have a description yet stay tuned for updates'
   var newBooks = _createBook(description, book.id, title, subtitle, authors, publishedDate, pageCount, thumbnail, amount, book.isOnSale)
        .then((book) => {
            gBooks.unshift(book)
            _saveBooksToStorage()
            return gBooks
...
function _createBook(description, id, title, subtitle, authors, publishedDate, pageCount, thumbnail, amount, isOnSale) {
    const book = {
        id,
        title,
        subtitle,
